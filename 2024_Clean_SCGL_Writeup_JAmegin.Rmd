...
# Final analysis of picnic for crossed design, still lacking the mark recapture but looks at two different directions
Set up data from raw data, don't worry about duplicated for now  
```{r}

picnic.obs_table <- picnic %>%
  mutate(detected = 1) %>%
  mutate(distance = abs(distance_to_group)) %>%
  rowwise() %>%
  mutate(size = sum(c(number_seedlings_in_group, number_vegetative_in_group, number_reproductive_in_group), na.rm = TRUE)) %>%
  ungroup() %>%
  dplyr::rename("Obs" = "transect_orientation") %>%
  dplyr::select(Obs, detected, ## detected by fill all plant_id for each trans_ori
         distance, size, transect_number) %>%
  relocate(size, .after = distance) %>%
  mutate(Sample.Label = paste0(transect_number, Obs)) %>%
  mutate(object = row_number()) %>%
  mutate(Region.Label = Obs)

picnic.sample_table <- picnic.samples %>%
  mutate(Region.Label = gsub("[^[:alpha:]]", "", Sample.Label))


picnic.region_table <- coveredarea %>%
  rename(Region.Label = Dir)
```


Miller et al 2019 Distance Sampling    
```{r}

### size
pic.size <- ds(picnic.obs_table, trunc = 3, key = "hr", formula = ~ size,
          region_table = picnic.region_table,
          sample_table = picnic.sample_table)
summary(pic.size)
plot(pic.size)
gof_ds(pic.size)  ## non significant goodness of fit, is a plausible model

pic.size.hn <- ds(picnic.obs_table, trunc = 3, key = "hn", formula = ~ size,
          region_table = picnic.region_table,
          sample_table = picnic.sample_table)
summary(pic.size.hn)
plot(pic.size.hn)
gof_ds(pic.size.hn)  ## non significant goodness of fit, is a plausible model


### nothing 
pic.nothing <- ds(picnic.obs_table, trunc = 3, key = "hr",
          region_table = picnic.region_table,
          sample_table = picnic.sample_table)
summary(pic.nothing)
plot(pic.nothing)
gof_ds(pic.nothing) ## non significant goodness of fit, is a plausible model


pic.nothing.hn <- ds(picnic.obs_table, trunc = 3, key = "hn",
          region_table = picnic.region_table,
          sample_table = picnic.sample_table)
summary(pic.nothing.hn)
plot(pic.nothing.hn)
gof_ds(pic.nothing.hn) ## non significant goodness of fit, is a plausible model

summarize_ds_models(pic.size, pic.size.hn, pic.nothing, pic.nothing.hn)
### If AIC less than 2, then investigate multiple best models
# Cramer-von Mises
```
...
detections_all_unique <- detections_all %>%
  filter(detected == 1) %>%
  mutate(observer = as.factor(observer)) %>%
  ## if wish to have hours after sunrise, size here? have a non-linear effect on detectability, convert to factor
...
in.mr.dist <- ddf(
                  dsmodel = ~ mcds(key = "hr", formula = ~ -observer + size),
                  # mrmodel = ~ glm(link = 'logit', formula = ~ distance + size),
                  data = detections_all_unique[,c("object","observer","distance","size")], # detections_all, # [detections_all$observer == "EW",], 
                  method = "ds",
...
coveredarea
picnic.samples
pic.data <- picnic.samples %>%
  mutate(Dir = gsub("[^[:alpha:]]", "", Sample.Label)) %>%
  mutate(Region.Label = paste0(Region.Label, Dir)) %>%
  left_join(coveredarea)

Picnic.det <- detections %>%
  filter(detected == 1) %>%
  left_join(coveredarea, by = c("observer" = "Dir"))


Picnic.hr <- ds(Picnic.det[,c("distance","size","Area")], truncation = 3, key = "hr", formula = ~ size)
summary(Picnic.hr)

Picnic.strata <- Picnic.det %>%
  rename("Dir" = "observer")


## Detection was increased a bit 0.731 by being from NS than EW
Picnic.hrDir <- ds(Picnic.strata[,c("distance","size","Area","Dir")], truncation = 3, key = "hr", formula = ~ size + Dir)
summary(Picnic.hrDir)
...
Try one at a time    
EW gives me node inconsistent with parents, not NS. NS just doesn't work
```{r}

picnic_simple <- detections_all %>%
  filter(detected == 1)

x <- picnic_simple$distance[picnic_simple$observer == "EW"]
...
jags.data <- list(nind=nind, nz=nz, x=x, y=y, B=B, A = coveredarea$Area[coveredarea$Dir == "EW"])
