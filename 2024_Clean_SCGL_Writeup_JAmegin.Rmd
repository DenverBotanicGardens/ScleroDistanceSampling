...
##??????????????
        ## Loop over sites (EW and NS)
        # for(s in 1:nsites){

        # Likelihood 
        # for(i in 1:(nind + nz)) {
        #   # Process model
        #   z[i] ~ dbern(psi)   # DA variables, zero if not 'real', 1 if included
        #   x[i] ~ dunif(0,B)   # Distribution of distances, assuming they are uniformly distributed, here's where to put if not? Yes! I think
        #   
        #   # Observation model
        #   p[i] <- exp(-d[i]*d[i]/(2*sigma[site[i]]*sigma[site[i]])) # Det. function
        #   mu[i] <- z[i]*p[i]  # 'straw man' for WinBUGS???
        #   # mu[i] <- z[i] * p[site[i]]                # p depends on site 
        #   y[i] ~ dbern(mu[i]) # Simple Bernoulli measurement error process for detection
        #   site[i] ~ dcat(site.probs[1:nsites])    # Population distribution among 'sites' so want to average this at the end
        #   
        #   # Observation model
        #   # log(p[s,i]) <- -((x[i]*x[i])/(2*sigma*sigma)) # Half-normal detection fct.
        #   # p[i] <- exp(logp[i])
        #   
        #   # Linear models for abundance and detection
        #     for(s in 1:nsites){
        #       # Model for abundance
        #       N[s] ~ dpois(lambda[s])            # realized abundance at site EW and then NS
        #       log(lambda[s]) <- alpha + beta*site[i]   # linear model abundance
        #       site.probs[s] <- lambda[s]/sum(lambda[])
        #       Navg <- mean(N[])
        #     
        #     # Linear model for detection
        #       log(sigma[s]) <- alpha + beta*site[i]
        #   }
        ...
        # Derived quantities
        N <- sum(z[1:(nind+nz)])  # Population size of seen and augmented
        D <- N/5400              # Density, with A = (100 * length(adddetection) * (2*strip.width)) meters^2
        
        }")

writeLines(modelPicnic, "PicnicDSall.jags")

## Inits function
zst <- y   # Augmented start with observed value of y
inits <- function(){ list(psi = runif(1), z = zst, sigma = runif(1, .4, 2))}  # Was 40 and 200 for 500 width

## Parameters to save
params <- c("N", "Navg","sigma", "D")
