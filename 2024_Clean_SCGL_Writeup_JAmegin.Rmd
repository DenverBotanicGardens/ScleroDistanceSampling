...
x <- detections_all$distance
nind <- length(x)
nz <- 500 
y <- c(detections_all$detected, rep(0,nz))
zoned <- c(detections_all$zoned, rep(NA, nz))
B <- 3                                              # strip half-width

## FIRST attempt without spatial of NS and EW
jags.data <- list(nind = nind, nz = nz, x = x, y = y, B = B)
...
modelPicnic <- 
  paste("
        ...
        # Priors  
        alpha ~ dunif(-10,10)   # intercept for EW vs. NS
        beta ~ dunif(-10,10)    # slope for EW vs. NS
        sigma ~ dunif(0, 1000)  # Half-normal scale, too big for ours? 
        # psi ~ dunif(0,1)        # Data Augmentation parameter (to include as real or not)
        ...
        for(i in 1:(nind + nz)) {
          ...
          site[i] ~ dcat(site.probs[1:nsites])      # site (EW or NS) membership of individuals
        }
