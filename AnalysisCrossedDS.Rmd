...
# haz_Null <- distsamp(~1 ~1, umf, keyfun="hazard", output="density",unitsOut="kmsq", starts = c(qlogis(0.25),log(12)))
# haz_Strata <- distsamp(~1 ~Strata, umf, keyfun="hazard", output="density",unitsOut="kmsq")

names(hn_Null)

## Predict to get esimates and 95% confidence intervals from the model on the original scale
...
#      cex.axis = 0.7, las = 1)
hist(hn_Strata, xlab = "Distance (m)", ylab = "Probability density", main = "")
plot(function(x) dxhn(x, sigma = 1.3909), 0,3,add=TRUE,  lwd = 2,col = "#1F70B7")
plot(function(x) dxhn(x, sigma = 1.26471), 0,3, lwd = 2, add=TRUE, col = "#004D40")
legend('topright', c("EW","Pooled","NS"),
       col = c("#1F70B7","black", "#004D40"), lty=1, lwd = 2,cex=1)
# dev.off()
```
