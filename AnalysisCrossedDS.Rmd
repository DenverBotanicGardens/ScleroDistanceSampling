...
CDS <- picnic %>%
  ...
  dplyr::mutate(length = case_when(transect_number == 1 & strata == "EW" ~ 70,
                            ...
                            TRUE ~ length)) %>%
  filter(size > 0) %>%
  select(transect, strata, distance, size, status, length) # %>%
...
CDS %>% 
  ...
  ggplot( aes(size, fill = DistClass))+
    geom_histogram(binwidth = 0.5, position = "dodge") +
    scale_fill_manual(Distance/nClass,values = c("#A78623", "#4A8CC5")) +
    theme_bw() +
    scale_x_continuous(breaks = 1:10) +
    facet_wrap(~ strata)
...
yDat <- formatDistData(as.data.frame(CDS), distCol = "distance",
...
pb <- parboot(hn_Strata, statistic=getN.hat, nsim=25)
pb
